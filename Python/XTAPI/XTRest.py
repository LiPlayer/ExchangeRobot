...
class XTCommon(APIBase):
    ...
    def _on_all_crypto_pairs_replied(self, reply: QNetworkReply):
        ...
        data = reply.readAll().data()
        json_data = json.loads(data.decode('utf-8'))
        json_data = json_data['result']['symbols']

        def convert(pair: dict) -> CryptoPair:
            fields = ['stateTime', 'nextStateTime']
            open_time = next((pair[field] for field in fields if pair.get(field) is not None), 0)
            return CryptoPair(
                exchange='XT',
                base=pair['baseCurrency'].upper(),
                quote=pair['quoteCurrency'].upper(),
                exchange_logo='https://altcoinsbox.com/wp-content/uploads/2023/01/xt.com-logo.svg',
                base_logo=pair['baseCurrencyLogo'],
